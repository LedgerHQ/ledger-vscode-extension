...
export async function showBuildMode() {
  let result = undefined;
  result = await vscode.window.showQuickPick(validBuildMode, {
    placeHolder: "Please select a build mode",
  });
  if (result) {
    buildMode.selected = result.toString();
